...
#from fmrate_corrections import fmrateerrorStack ###SSS 7/2018: For fm rate error corrections
import re
...
%%%%%%%%%%%%%%%
Example 5:
# interferogram workflow with fmrate corrections *note, only compatible with geometry coregistration

stackSentinel.py -s ../SLC/ -d ../../MexicoCity/demLat_N18_N20_Lon_W100_W097.dem.wgs84 -b '19 20 -99.5 -98.5' -a ../../AuxDir/ -o ../../Orbits -C geometry -W fmratecorrection

%%%%%%%%%%%%%%%
Example 6:
# interferogram workflow with layover and water mask applied to filtered phase

stackSentinel.py -s ../SLC/ -d ../../MexicoCity/demLat_N18_N20_Lon_W100_W097.dem.wgs84 -b '19 20 -99.5 -98.5' -a ../../AuxDir/ -o ../../Orbits -layover_msk -water_msk
...
def createParser():
    ...
    parser.add_argument('-ilist', action='store', default=None, dest='ilist',
	    help='File containing pairs the user wishes to process. Each pair is listed on an individual line in a YYYYMMDD_YYYYMMDD format.', type=str)  ###SSS 7/2018
    ...
    parser.add_argument('-use_VRT', '--use_virtual_files', action='store_false', dest='useVirtualFiles', ###SSS 7/2018: virtual option added
            default=True, help = 'writing only vrt of merged files (Default: True). If flag is passed, bursts are physically unpacked.')
...
def get_dates(inps):
    ...
    if os.path.isfile(inps.slc_dirname):
        ...
        SAFE_files = glob.glob(os.path.join(inps.slc_dirname,'S1*_IW_SLC*')) # changed to zip file by Minyan Zhong ###SSS 7/9/18: Changed from *zip in case you have unpacked SAFE files.
...
def selectNeighborPairs(inps, dateList, num_connections, updateStack=False):  # should be changed to able to account for the existed aquisitions -- Minyan Zhong
    ...
    if os.path.exists(existing_ifgs):
        OG_pairs=[ name for name in os.listdir(existing_ifgs) if os.path.exists(os.path.join(existing_ifgs, name, 'filt_fine.int'))]
        for i in OG_pairs:
            if (i[:8],i[9:]) in pairs:
...
def customPairs(ilist,ilistonly,work_dir,exclude_dates,pairs,acquisitionDates, safe_dict_og, master_date, slaveDates, allSLCs): ###SSS 7/9/2018: Add custom pairs.

    if ilist is None and ilistonly:
        print("ERROR: By specifying '-ilistonly', you may process only the pairs you list within a file passed through with the 'ilist' flag. \nBut it appears that you did not provide a valid file input for the 'ilist' flag!!!!!!")
        sys.exit(1)
...
def interferogramStack(inps, acquisitionDates, stackMasterDate, slaveDates, safe_dict, pairs, updateStack, allSLCs):
    ...
    runObj = run()
    runObj.configure(inps, 'run_' + str(i) + "_merge_burst_igram") 
    runObj.burstIgram_mergeBurst(allSLCs, safe_dict, pairs, inps) ###SSS 7/2018: clean-up fine*int, if specified.
    runObj.finalize()
    ...
    i+=1 ###SSS 7/2018: Geometry step moved earlier on in chain so users may use water mask, if specified.
    runObj = run()
...
def fmrateerrorStack(inps, acquisitionDates, stackMasterDate, slaveDates, safe_dict, pairs, updateStack, allSLCs):

    i = slcStack(inps, acquisitionDates, stackMasterDate, slaveDates, safe_dict, updateStack)

    i+=1
    runObj = run()
    runObj.configure(inps, 'run_' + str(i) + "_merge_burst_igram_fmrateerror") 
    runObj.burstIgram_mergeBurstfmrateerror(allSLCs, safe_dict, pairs, inps) ###SSS 7/2018: clean-up fine*int, if specified.
    runObj.finalize()
...
def main(iargs=None):
    ...
    if inps.workflow not in ['interferogram', 'offset', 'correlation', 'slc', 'fmratecorrection']: ###SSS 7/9/18: FMrate error correction.
        print('')
        print('**************************')
    ...
    if inps.workflow == 'fmratecorrection': ###SSS 7/9/2018: If you wish to perform fm rate corrections, do first checks.
        inps.coregistration == 'geometry'
        print('')
        print('**************************')
    ...
    if inps.workflow == 'interferogram':
        ...
        interferogramStack(inps, acquisitionDates, stackMasterDate, slaveDates, safe_dict, pairs, updateStack, allSLCs)  
    ...
    elif inps.workflow == 'fmratecorrection':  ###SSS 7/9/18: FMrate error correction.
        
        fmrateerrorStack(inps, acquisitionDates, stackMasterDate, slaveDates, safe_dict, pairs, updateStack, allSLCs)  
