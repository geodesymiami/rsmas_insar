...
def generate_url_download(date, extension):
    ...
    if extension == 'nc4':
        url = 'https://data.gesdisc.earthdata.nasa.gov/data/GPM_L3/GPM_3IMERGDF.06/' + year + '/' + month + '/3B-DAY.MS.MRG.3IMERG.' + year+month+day + '-S000000-E235959.V06.' + extension
    else:
        url = 'https://data.gesdisc.earthdata.nasa.gov/data/GPM_L3/GPM_3IMERGM.07/' + year + '/3B-MO.MS.MRG.3IMERG.'+ year+month+day +'-S000000-E235959.08.V07B.' + extension
...
def precipitaion_nc4(longitude, latitude, date_list, folder):
...
if __name__ == "__main__":
    ...
    if args.download:
        date_list = generate_date_list(args.download[0], args.download[1])
        ...
        if args.plot_daily:
            ...
            date_list = generate_date_list(args.plot_daily[2], args.plot_daily[3])

        elif args.plot_weekly:
            la = round(float(args.plot_weekly[0]), 1)
            lo = round(float(args.plot_weekly[1]), 1)
            date_list = generate_date_list(args.plot_weekly[2], args.plot_weekly[3])

        else:
            print('Specify a plot frequency')

        check_nc4_hdf5_new(work_dir, date_list)
        prec = precipitaion_nc4(lo, la, date_list, work_dir)
