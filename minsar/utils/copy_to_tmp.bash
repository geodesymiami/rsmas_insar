...
if [[ $job_name == *"run_02_unpack_secondary_slc"* ]]; then
    ...
    for date in "${date_list[@]}"; do
        distribute.bash $out_dir/secondarys/$date /tmp/secondarys; mv /tmp/$date /tmp/secondarys
    done
    ...
    mkdir -p /tmp/secondarys
    for date in "${date_list[@]}"; do
        distribute.bash $out_dir/secondarys/$date; mv /tmp/$date /tmp/secondarys
    done
    files1="/tmp/secondarys/????????/*.xml"
    files2="/tmp/secondarys/????????/*/*.xml"
    old=$out_dir
    ...
    # reference
    distribute.bash $out_dir/reference; files="/tmp/reference/*.xml /tmp/reference/*/*.xml"
    old=$out_dir 
    sed -i "s|$old|/tmp|g" $files
    ...
    # reference
    distribute.bash $out_dir/reference /tmp
    files="/tmp/reference/*.xml /tmp/reference/*/*.xml"
    old=$out_dir 
    sed -i "s|$old|/tmp|g" $files
